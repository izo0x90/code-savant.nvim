            ...
            if resolved_path.is_file():
                try:
                    ...
                    raise SettingsLoadError(
                        f"Failed to read API key from configured file '{self.api_key_file}'",
                        path=resolved_path,
                        details=str(e),
                    ) from e
...
class ModelCapabilityConfig(BaseModel):
    ...
    supports_tools: bool = Field(
        default=True, description="True if model supports native tool/function calling."
    )
...
class ModelLimitConfig(BaseModel):
    ...
    context_tokens: int = Field(
        ..., gt=0, description="Total available input + output token window."
    )
...
class EngineDaemonSettings(BaseSettings):
    ...
    strategy_config: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Extensible settings indexed by strategy identifiers.",
    )
    ...
    @field_validator("session_storage_dir", mode="after")
    @classmethod
    def expand_session_storage_dir(cls, v: str) -> str:
        """Type-safe path expansion utilizing Pydantic validator instead of raw dict logic."""
        return str(Path(v).expanduser())
...
class SettingsManager:
    ...
    def settings(self) -> EngineDaemonSettings:
        ...
        if self._settings is None:
            raise SettingsLoadError(
                "Settings accessed before loading. Call load_settings() first."
            )
    ...
    async def load_settings(self) -> EngineDaemonSettings:
        ...
        if not self.default_path.is_file():
            raise SettingsLoadError(
                "Critical default settings file is missing.", path=self.default_path
            )

        # Read default settings asynchronously
        try:
            async with aiofiles.open(
                self.default_path, mode="r", encoding="utf-8"
            ) as f:
                content = await f.read()
        ...
        if user_resolved_path.is_file():
            try:
                async with aiofiles.open(
                    user_resolved_path, mode="r", encoding="utf-8"
                ) as f:
                    content = await f.read()
                # Validate user overrides schema using Partial helper model
                user_overrides = PartialDaemonSettings.model_validate_json(
                    content
                ...
                raise SettingsLoadError(
                    "Global user settings file is corrupted.",
                    path=user_resolved_path,
                    details=str(e),
                ) from e
        ...
        if project_resolved_path.is_file():
            try:
                async with aiofiles.open(
                    project_resolved_path, mode="r", encoding="utf-8"
                ) as f:
                    content = await f.read()
                # Validate project overrides schema using Partial helper model
                project_overrides = PartialDaemonSettings.model_validate_json(
                    content
                ).model_dump(exclude_unset=True)
                deep_merge(merged_dict, project_overrides)
            except Exception as e:
                raise SettingsLoadError(
                    "Project local settings file is corrupted.",
                    path=project_resolved_path,
                    details=str(e),
                ) from e
        ...
        try:
            ...
            raise SettingsLoadError(
                "Schema validation failed for merged configuration cascade.",
                details=str(e),
            ) from e
