...
class AsyncToolScheduler:
    ...
    async def schedule(self, function_calls: List[ToolCall], prompt_id: str, context: ExecutionContext) -> ToolExecutionOutcome:
        tool_responses = []
        ...
        for call in function_calls:
            ...
            if not tool:
                resp = {"error": f"Unauthorized tool call: '{name}' is not available to this agent."}
            else:
                ...
                if self.requires_approval:
                    guards.append(UserConfirmationGuard(
                        timer=self.timer,
                        is_interactive=True,
                        call_id=call_id
                    ))
...
class LocalAgentExecutor:
    ...
    async def execute_turn(
        ...
        sanitized_user_msg = await truncator.truncate_if_needed(user_msg, context.workspace_path)
        
        next_message = sanitized_user_msg
        await context.session.append_message(sanitized_user_msg)

        # Trigger session save via manager, letting the repository handle dynamic metadata
    ...
    async def execute_final_warning_turn(self, turn_counter: int, context: ExecutionContext, reason: str) -> Optional[str]:
        ...
        try:
            turn_result = await self.execute_turn(user_warning, turn_counter, grace_timer, context)
    ...
    async def run(self, context: ExecutionContext, inputs: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            while True:
                ...
                turn_result = await self.execute_turn(current_message, turn_counter, deadline_timer, context)
                turn_counter += 1
                ...
                if self.pending_hints_queue:
                    ...
                    current_message = ChatMessage(
                        ...
                        parts=[hint_part] + current_message.parts
                    )
                    await context.session.set_history(context.session.chat_history[:-1] + [current_message])

        finally:
            deadline_timer.stop()

        if terminate_reason in [TerminationReason.REASON_MAX_TURNS.value, TerminationReason.REASON_TIMEOUT.value]:
            recovery_output = await self.execute_final_warning_turn(turn_counter, context, terminate_reason)
            if recovery_output:
                terminate_reason = TerminationReason.REASON_GOAL.value
