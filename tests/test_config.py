...
from engine.config import SettingsManager, EngineDaemonSettings, SettingsLoadError, ProviderSettings
from engine.registry import ModelRegistryService, ModelDefinition, ModelRegistryError
...
async def test_settings_manager_precedence_cascade(tmp_path: Path):
    ...
    # Instantiate manager
    mgr = SettingsManager(default_path=default_file, user_path=user_file, project_path=proj_file)
    ...
    assert settings.small_model == "google/gemini-3.5-flash"  # Inherited from user overrides
...
async def test_settings_manager_missing_defaults_fails_loudly(tmp_path: Path):
    ...
    mgr = SettingsManager(default_path=non_existent, user_path=tmp_path / "user.json", project_path=tmp_path / "proj.json")
...
async def test_settings_manager_corrupted_json_fails_loudly(tmp_path: Path):
    ...
    mgr = SettingsManager(default_path=default_file, user_path=user_file, project_path=tmp_path / "proj.json")
...
async def test_model_registry_atomic_persistence(tmp_path: Path):
    ...
    assert not tmp_path.joinpath("cache.tmp").exists()  # Temp file has been cleaned up/swapped

    # Boot a secondary registry pointing to the same files to verify persistent caching
    registry_reboot = ModelRegistryService(bundled_path=bundled_file, cache_path=cache_file)
...
async def test_settings_manager_dump_default_settings(tmp_path: Path):
    ...
    mgr = SettingsManager(default_path=default_file, user_path=tmp_path / "user.json", project_path=tmp_path / "proj.json")
    ...
    assert loaded_data["socket_path"] == str(Path("~/.cache/code_savant/engine.sock").expanduser())
    ...
    missing_mgr = SettingsManager(default_path=tmp_path / "nonexistent_defaults.json", user_path=tmp_path / "user.json", project_path=tmp_path / "proj.json")
    with pytest.raises(SettingsLoadError) as exc_info:
